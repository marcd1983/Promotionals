...
class PromoPageController extends PageController
{
    ...
    public function PromoForm(): Form
    {
        ...
        $fields = FieldList::create(
            ...
            // context (hidden)
            HiddenField::create('PromoSlug', '')
                ->setValue($promo?->URLSegment ?? ''),
            HiddenField::create('PromoTitle', '')
                ->setValue($promo?->Title ?? ''),
            HiddenField::create('PromoLink', '')
                ->setValue($promo?->Link() ?? $this->Link()),
    ...
    public function doSubmitPromoForm(array $data, Form $form)
    {
        ...
        $crm->captureLead($data, $this->getRequest(), [
            'source'       => $data['CRMSource'] ?? 'promo-enquiry',
            'page_url'     => $data['PageURL'] ?? $promoLink,
            'promo_id'     => $promo?->ID,
